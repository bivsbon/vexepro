    ...
    ?>
        <main>
            ...
            $script = "<script type='text/javascript'>
                const ticket_status = {'pending': 'Mới đặt', 'active' : 'Kích hoạt', 'canceled' : 'Hủy', 'used' : 'Đã sử dụng'};
                const tickets = [";
                            ...
                            \${(function fun(){
                                ...
                                tickets.filter(item => item.status === 'pending').forEach(item =>{
                                     ct += `<div class='ticket-item-wrapper'>
                                                <div class='ticket-id' style='margin-bottom: 8px'>No.\${item.id} <span style='font-size: 12px;padding: 2px 4px; background:white; text-transform: uppercase; border-radius:5px; color:black'>\${ticket_status[item.status]}</span></div>    
                                                ...
                                                <a href='/vexepro/ticket/cancel?ticket_id=\${item.id}' class='button' style='background: #FF7F7F; margin: 8px 0px; color: white'>Hủy vé</a>
                                               
<p>
<button class='button' type='' style='background: green; margin: 8px 0px; color: white'
onClick='showDialog(\${item.id})'
>Yêu cầu kích hoạt</button>
</p>
                                           </div>`
                                });
                                return ct;
                    ...
                    },
                    {
                        ...
                        <div>
                            \${(function fun(){
                                let ct = '';
                                tickets.filter(item => item.status === 'canceled').forEach(item =>{
                                    ct += `<div class='ticket-item-wrapper'>
                                                <div class='ticket-id' style='margin-bottom: 8px'>No.\${item.id} <span style='font-size: 12px;padding: 2px 4px; background:red; text-transform: uppercase; border-radius:5px; color:white'>\${ticket_status[item.status]}</span></div>    
                                                <div class='ticket-content'>
                                                    <div class='ticket-content-l'>
                                                        <div style='font-size:18px'><b>Người mua:</b><i> \${item.owner}</i></div>
                                                        <div><b>Nhà xe:</b> \${item.agency_name}</div>
